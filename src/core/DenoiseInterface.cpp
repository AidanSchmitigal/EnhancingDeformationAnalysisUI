...
bool DenoiseInterface::Denoise(std::vector<uint32_t *> &images, int width,
	...
	for (int i = 0; i < images.size(); i++) {
			...
			for (int y = 0; y < tile.data.rows; y++)
				for (int x = 0; x < tile.data.cols; x++)
					image_data.push_back(
					    tile.data.at<float>(y, x));

			cppflow::tensor input = cppflow::tensor(
			    image_data, {1, tile.data.rows, tile.data.cols, 1});

			try {
				auto output2 = model(
				    {{"serving_default_input_gen", input}},
		...
		for (int j = 0; j < tiles.size(); j++) {
			...
			for (int y = 0; y < tiles[j].data.rows; y++) {
				for (int x = 0; x < tiles[j].data.cols; x++) {
					output_image.at<float>(y, x) =
					    output_data[y * tiles[j].data.cols +
							x];
				}
			}
		...
		cv::Mat reconstructed =
		    Tiler::StitchTiles(tiles, config, image.size(), true);
...
std::future<bool>
DenoiseInterface::DenoiseAsync(std::vector<uint32_t *> &images, int width,
			       int height, const std::string &model_name,
			       const TileConfig &config,
	...
	auto future = pool.enqueue(
	    [&images, width, height, model_name, config, callback]() {
		    bool result =
			Denoise(images, width, height, model_name, config);
...
bool DenoiseInterface::Blur(std::vector<uint32_t *> &images, int width,
			    int height, int kernel_size, float sigma) {
	PROFILE_FUNCTION();
	...
	for (int i = 0; i < images.size(); i++) {
		...
		cv::Mat output_image;
		cv::GaussianBlur(image, output_image,
...
std::future<bool>
DenoiseInterface::BlurAsync(std::vector<uint32_t *> &images, int width,
			    int height, int kernel_size, float sigma,
			    std::function<void(bool)> callback) {
	// Set processing flag
	m_is_processing = true;
	...
	// Submit task to thread pool
	auto future = pool.enqueue([&images, width, height, kernel_size, sigma,
				    callback]() {
