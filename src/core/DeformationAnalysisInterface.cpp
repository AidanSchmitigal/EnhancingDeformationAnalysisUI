...
inline std::vector<Tile> split_tiles(const cv::Mat &img, int tileSize = 256,
				     int overlap = 0) {
	if (img.empty())
		throw std::runtime_error("split_tiles: empty input");
	int step = tileSize - overlap;
	if (step <= 0)
		throw std::runtime_error(
		    "split_tiles: overlap must be < tileSize");
	std::vector<Tile> out;
	for (int y = 0; y < img.rows; y += step) {
		...
		if (y + std::min(tileSize, img.rows - y) >= img.rows)
			break;
	}
	return out;
}
...
inline cv::Mat stitch_tiles(const std::vector<Tile> &tiles, cv::Size original,
			    int tileSize = 256, int overlap = 0) {
	if (tiles.empty())
		throw std::runtime_error("stitch_tiles: no tiles");
...
bool DeformationAnalysisInterface::RunModel(std::vector<uint32_t *> &images,
	...
	auto to_tensor = [](const cv::Mat &m) {
		return torch::from_blob(m.data, {1, 1, 256, 256}, torch::kUInt8)
		    .to(torch::kFloat32)
		    .clone();
	};
	...
	for (int i = 0; i + 1 <= images.size() - 1; ++i) {
		...
		for (int k = 0; k < tiles.size(); ++k) {
			...
			auto input = torch::cat({to_tensor(tiles[k].data),
						 to_tensor(tiles2[k].data)},
						1)
					 .to(dev);

			auto out = model.forward({input}).toTensor().to(
			    torch::kCPU); // out: torch::Tensor on cpu, shape
					  // 1,2,H,W
			...
			outTiles.push_back(
			    {bgr.clone(), tiles[k].position}); // own memory
...
bool DeformationAnalysisInterface::TestModelCPPFlow(
    std::vector<uint32_t *> &images, int width, int height, const int tile_size,
	...
	const std::string in_op =
	    "serving_default_input:0"; // verify via saved_model_cli
		...
		for (size_t k = 0; k < tiles1.size(); ++k) {
			...
			for (int y = 0; y < tile_size; ++y)
				for (int x = 0; x < tile_size; ++x) {
					buf[(y * tile_size + x) * 2 + 0] =
					    tiles1[k].data.at<uchar>(y, x);
					buf[(y * tile_size + x) * 2 + 1] =
					    tiles2[k].data.at<uchar>(y, x);
				}
			cppflow::tensor input(buf,
			...
			for (int y = 0; y < tile_size; ++y)
				for (int x = 0; x < tile_size; ++x) {
					...
					mr.at<uchar>(y, x) =
					    norm_u8(data[idx * 2 + 0]);
					mg.at<uchar>(y, x) =
					    norm_u8(data[idx * 2 + 1]);
				}
			...
			output_tiles.push_back(
			    {bgra.clone(), tiles1[k].position});
		}
		auto stitched = stitch_tiles(outTiles, img1.size(), tile_size);
		cv::imwrite("cppflow_stitched" + std::to_string(i) + ".png",
			    stitched);
		memcpy(images[i], stitched.data,
		       width * height * sizeof(uint32_t));
	}
